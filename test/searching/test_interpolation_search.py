import pytest
from algo.searching import interpolation_search
...
class TestExponentialSearch:
  ...
  def test_solution_found(self, dataToSearch, expected):
    assert interpolation_search.solution(self.testArr, dataToSearch) == expected
  
  def test_solution_not_found(self):
    assert interpolation_search.solution(self.testArr, 17) == -1